...
export default function PhysicsExams() {
  const [exams, setExams] = useState<LinkItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const subjectName = 'physics'; // Define the subject for this page
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-2 text-gray-700">Physics Exams</h1>
      <p className="text-gray-700 mb-8">Select an exam to begin the timed test</p>
      
      <div className="grid gap-6">
        {exams.length === 0 && !isLoading && (
          <p className="text-gray-500">No physics exams found.</p>
        )}
        {exams.map((exam) => {
          ...
          const duration = 75; // Default for physics
          const questions = 25; // Default for physics
