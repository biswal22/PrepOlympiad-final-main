...
export default function ChemistryExams() {
  const [exams, setExams] = useState<LinkItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const subjectName = 'chemistry'; // Define the subject for this page
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Filter for chemistry questions
        const chemistryExams = links.filter(
          link => link.subject === subjectName && link.category === 'questions'
        );
        setExams(chemistryExams);
      } catch (e) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-2 text-gray-700">Chemistry Exams</h1>
      <p className="text-gray-700 mb-8">Select an exam to begin the timed test</p>
      
      <div className="grid gap-6">
        {exams.length === 0 && !isLoading && (
          <p className="text-gray-500">No chemistry exams found.</p>
        )}
        {exams.map((exam) => {
          ...
          // Add placeholders for duration/questions/difficulty as they are not in JSON
          const duration = 110; // Default for chemistry
          const questions = 60;  // Default for chemistry
          const difficulty = 'Intermediate'; // Placeholder

          return (
            <Link
              key={examUrlId} // Use the generated ID for the key
              href={`/subjects/${subjectName}/exams/${examUrlId}`} // Use the generated ID in the URL
